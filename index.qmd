...
- **Structured additive predictor for location parameter**  
  $$
  \log(\mu_i) = \beta_0 + \beta_1 \text{temperature}_i + u_{\text{region}[i]} + \varepsilon_i
  $$
- **What's great:** Location-specific location parameter with spatial dependence, while maintaining computational tractability for 43,920 locations.
- **What we're missing:** Spatial dependence for scale, shape and trend parameters.
:::
...
- **Joint GEV model for precipitation extremes**  
  1) *Data:* $y_i \mid \mu_i, \sigma_i, \xi_i, \Delta_i \sim \text{GEV}(\mu_i, \sigma_i, \xi_i, \Delta_i)$  
  ...
  $\qquad(\psi_i, \tau_i, \phi_i, \gamma_i) = (\log \mu_i, \log \sigma_i - \log \mu_i, f(\xi_i),  \log \Delta_i)$
  3) *Hyper-parameters:* $\theta =$ e.g. mean, range, variance, spatial correlation
:::
...
![](images/copula/psi.png)

##

![](images/copula/mu.png)

##

![](images/copula/tau.png)

##

![](images/copula/sigma.png)

##

![](images/copula/phi.png)

##

![](images/copula/xi.png)

##

![](images/copula/gamma.png)

##

![](images/copula/Delta.png)
